...
const Signup = ({ variables }) => {
  const { setLogin, handleName2, handleUsername2, handlePassword2, handleCpassword2, name2, username2, password2, cpassword2, seeSignup, setSeeSignup, seeSignup2, setSeeSignup2, handleSubmit2, open, setOpen, alertStatement, setAlertStatement } = variables;
  return (
    <Box>
        <Card sx={{ boxShadow: '0px 0px 20px 0px rgba(0,0,0,0.75)', width: { sm: '500px', xs: '90vw' } }}>
            <CardContent>
              ...
              <Box sx={{ display: 'flex', flexDirection: 'row', alignItems: 'center', justifyContent: 'center' }} mt={2}>
                ...
                <Button onClick={()=>{ setLogin(true); setAlertStatement(''); setOpen(false); }}>Login</Button>
              </Box>
            </CardContent>
        </Card>
    </Box>
